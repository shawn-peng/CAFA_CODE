...
%             model_score.color = colors{i};
        ...
        groups{i} = {groups{i}.seq_smin_bst};
        group_array = [groups{i}{:}];
        [group_sel, ~, info] = cafa_sel_top_seq_smin(ntop, groups{i}, '', '', [cfg_dirs{1}, 'register.tab'], false);
        ...
        for ii = 1:ntop
            ...
            groups_top(i, ii).avgsmins  = nanmean([group_top(ii).smin_bst]);
            % matrix percentile
            groups_top(i, ii).smin_q05  = prctile([group_top(ii).smin_bst], 5);
            groups_top(i, ii).smin_q95  = prctile([group_top(ii).smin_bst], 95);
...
%     end
    ...
    [~, rank_idx] = sort([all_in_one.avgsmins], 'ascend');
    ...
    ylim_u = ylim(2);
    for i = 1:ntop
        rpos = [i - bar_w / 2, ylim_l, bar_w, data(i).avgsmins - ylim_l];
...
%     bsl_res{5} = bsl_scores{1,3}.score.seq_smin_bst;
%     bsl_res{6} = bsl_scores{2,3}.score.seq_smin_bst;
%     bsl_res{7} = bsl_scores{1,4}.score.seq_smin_bst;
%     bsl_res{8} = bsl_scores{2,4}.score.seq_smin_bst;
    bsl_scores = bsl_scores';
    for i = 1:numel(res_dirs)
        b = 1;
        bsl_res = bsl_scores{b, i}.score.seq_smin_bst;
        bsl_data.avgsmins = nanmean(bsl_res.smin_bst);
        bsl_data.smin_q05  = prctile([bsl_res.smin_bst], 5);
        bsl_data.smin_q95  = prctile([bsl_res.smin_bst], 95);
        bsl_data.coverage  = nanmean([bsl_res.coverage_bst]);
%             name_rank = arrayfun(@num2str, [1:ntop]', 'UniformOutput', 0);
        bsl_data.disp_name = strcat(cafas{i}, '_Naive');
        bsl_data.color = bcolor{b};
        j = ntop + (i - 1)*2 + b;
        rpos = [j - bar_w / 2, ylim_l, bar_w, bsl_data.avgsmins - ylim_l];
        rectangle('Position', rpos, 'FaceColor', bsl_data.color, 'EdgeColor', bsl_data.color);
        line([j, j], [bsl_data.smin_q05, bsl_data.smin_q95], 'Color', 'k');
        line([j - bar_w / 4, j + bar_w / 4], [bsl_data.smin_q05, bsl_data.smin_q05], 'Color', 'k');
        line([j - bar_w / 4, j + bar_w / 4], [bsl_data.smin_q95, bsl_data.smin_q95], 'Color', 'k');
...
%     ax.YTick              = ylim_l : 1 : ylim_u;
    ax.XTickLabel         = xticks;
    ax.FontSize           = base_fs;
    ax.XTickLabelRotation = 45;
    
%     cafa_plot_duel_smin([output_dir, ontname, '_duel_result.png'],duel_res,bsl_res,ontname)
    saveas(gcf,[output_dir, ontname, '_smin_cafas.png'])
    close
end
