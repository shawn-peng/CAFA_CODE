...
%             model_score.color = colors{i};
    ...
    group1 = {group1.seq_fmax_bst}
    group1_array = [group1{:}]
    [group1_sel, ~, info] = cafa_sel_top_seq_fmax(5, group1, '', '', [cfg_dirs{1}, 'register.tab'], false);
    [~, idx] = ismember(info.top_mid, {group1_array.id})
%     avgfmaxs = nanmean([group1.fmax_bst])
%     [~,idx] = sort(avgfmaxs, 'descend');
    for i = 1:5
        group1_top5{i} = group1{idx(i)};
    end

    group2 = [model_scores{:,2}.score]
    group2 = {group2.seq_fmax_bst}
    group2_array = [group2{:}]
    [group2_sel, ~, info] = cafa_sel_top_seq_fmax(5, group2, '', '', [cfg_dirs{2}, 'register.tab'], false);
    [~, idx] = ismember(info.top_mid, {group2_array.id})
%     avgfmaxs = nanmean([group2.fmax_bst])
%     [~,idx] = sort(avgfmaxs, 'descend');
    for i = 1:5
        group2_top5{i} = group2{idx(i)};
    end

    duel_res = cafa_duel_seq_fmax(group1_top5, group2_top5)
